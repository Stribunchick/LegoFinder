...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def connect_signals(self):
        """Подключить элементы интерфейса к соответствующим обработчикам."""
    ...
    def _on_start_stop_toggled(self, checked: bool):
        """Запустить или остановить получение кадров от текущего источника."""
        if checked:
            if not isinstance(self.app_controller.frame_grabber.source, CameraSource):
                self.app_controller.set_source(CameraSource(self.webcamsrc))
    ...
    def _on_load_image_button_clicked(self):
        """Открыть медиафайл и переключить источник на него."""
        self.app_controller.stop_pipeline()
        ...
        self.app_controller.set_source(source)
    ...
    def _on_conf_thres_slider_changed(self, value):
        """Передать значение порога уверенности из слайдера в обработчик."""
        self.app_controller.update_thres(value)

    @Slot()
    def _on_add_part_clicked(self):
        """Открыть окно добавления эталона и перенаправить туда кадры."""
        self.add_part_window = AddPartWindow(self, reference_folder=self.folder)
    ...
    def _update_frame(self, frame: np.ndarray):
        """Показать последний обработанный кадр в основном окне."""
        self.image_widget.update_frames(frame)

    @Slot(str)
    def select_part_from_list(self, det_name):
        """Переключить детектор на выбранный эталон детали."""
        self.app_controller.change_part(det_name)

    @override
    def closeEvent(self, event):
        """Остановить фоновые потоки перед закрытием окна."""
